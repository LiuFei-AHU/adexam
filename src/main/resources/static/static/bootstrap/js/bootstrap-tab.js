    ...
    BaseTab.prototype.addTab=function (obj) {
        ...
        if(obj.closeable){
            this.$element.find(".nav-tabs li a[href='#" + obj.id + "'] i.closeable").click(function () {
                var href = $(this).parents("a").attr("href").substring(1);
                if(self.getCurrentTabId()==href){
					//added by liufei 20210203 解决关闭tab后不显示其他tab的内容 --start
					var li = self.$element.find(".nav-tabs li")
					var last = li.length - 2
					console.log('last='+last)
                    self.$element.find(".nav-tabs li:eq("+last+") a").tab("show");
					$('#container .tab-content').children('div:eq('+last+')').addClass("active in");
					// --end
                }
                $(this).parents("li").remove();
                $("#" + href).remove();
